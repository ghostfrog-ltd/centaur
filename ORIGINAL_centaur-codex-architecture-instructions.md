...
db_path: storage/paper/centaur_paper.db
...
db_path: storage/live/centaur_live.db
...
```text
storage/paper/centaur_paper.db
storage/live/centaur_live.db
```
...
- daily live order count <= max_live_orders_per_day
- daily live loss has not breached max_daily_live_loss
- latest market data is fresh
...
4. Ensure DB path comes from config.
...
```text
- Centaur can run paper using configs/paper.yaml and paper DB.
...
- Paper and live use separate DB files.
- Paper fitness can be viewed/imported as evidence but is not mixed into live trades.
- Dashboard clearly shows mode, environment, DB path, broker/account type, and live guard status.
