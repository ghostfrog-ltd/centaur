...
$url = centaurRecentTradesApiUrl($hours, $brokerId);
...
if (($result['ok'] ?? false) !== true) {
    ...
    echo json_encode(
        [
            'ok' => false,
            'error' => 'recent_trades_api_unavailable',
            'detail' => 'The Centaur dashboard API did not return recent trade data.',
            'api_url' => $url,
...
function centaurRecentTradesApiUrl(int $hours, string $brokerId): string
{
    $snapshotUrl = centaurSnapshotApiUrl();
    $path = '/api/recent-trades?' . http_build_query(
        [
            'hours' => $hours,
            'broker_id' => $brokerId,
        ]
