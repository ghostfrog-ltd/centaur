...
class Trading212ApiError(RuntimeError):
    """Raised when the Trading 212 API cannot return a trusted payload."""


class Trading212PaperClient:
    """Minimal client for Trading 212's public API.

    Paper and live environments share endpoint shapes but must keep separate
    credentials, base URLs, usage provenance, and execution gates.
    """
    ...
    def __init__(
        ...
        *,
        ...
        base_url: str,
        timeout_seconds: int,
        usage_source: str = "trading212_paper",
    ) -> None:
        self.api_key = api_key
        self.api_secret = api_secret
        self.base_url = _normalize_base_url(base_url)
        self.timeout_seconds = int(timeout_seconds)
        self.usage_source = str(usage_source or "trading212_paper").strip() or "trading212_paper"
    ...
    def from_config(cls, config: RuntimeConfig) -> "Trading212PaperClient":
        ...
        return cls(
            ...
            usage_source="trading212_paper",
        )

    @classmethod
    def from_live_config(cls, config: RuntimeConfig) -> "Trading212PaperClient":
        if not config.trading212_live_api_configured:
            raise Trading212ApiError("Trading 212 live API credentials are not configured.")
        return cls(
    ...
    def get_positions(self, context: TickContext) -> list[dict[str, Any]]:
        try:
            payload = self._request_json(context=context, path="/equity/positions")
        ...
        *,
        ...
        payload = self._request_json(
            ...
            body_json={
                "quantity": order_request["quantity"],
                "ticker": order_request["ticker"],
                "extendedHours": bool(order_request.get("extendedHours", False)),
            },
        )
        if isinstance(payload, dict):
            return payload
        raise Trading212ApiError("Unexpected Trading 212 market order response payload.")
        ...
        *,
        ...
        try:
            ...
            raise Trading212ApiError(
                f"Trading 212 API HTTP {exc.code} on {path}: {exc.reason}{detail}"
            ) from exc
