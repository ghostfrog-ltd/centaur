...
class Trading212PaperBrokerAdapter(BrokerAdapter):
    """Trading 212 demo-account adapter for the separate paper equity lane.

    Trading 212 paper is a separate experimental account boundary, not a reason
    to blur Centaur's evidence. The adapter validates fixed-notional equity
    requests, reads account state, and mutates only after Centaur's paper gates.
    """
    ...
    def from_config(cls, config: RuntimeConfig) -> "Trading212PaperBrokerAdapter":
        return cls(
            ...
            equity_symbols=getattr(config, "trading212_paper_equity_symbols", tuple()),
    ...
    def _client(self) -> Trading212PaperClient:
        try:
            ...
            usage_source=self.broker_id,
        )
        except Trading212ApiError as exc:
            raise BrokerAdapterError(str(exc)) from exc

    def resolve_ticker(self, symbol: str, proposal: dict[str, Any] | None = None) -> str | None:
        normalized = str(symbol or "").strip().upper()
        if not normalized:
            return None
        explicit_ticker = self.ticker_overrides.get(normalized)
        if explicit_ticker:
            return explicit_ticker
        if proposal is not None:
            for key in ("venue_symbol", "ticker", "trading212_ticker"):
                candidate = str(proposal.get(key, "")).strip()
                if candidate:
                    return candidate
            instrument_ref = proposal.get("instrument_ref")
            if isinstance(instrument_ref, dict):
                venue_symbol = str(instrument_ref.get("venue_symbol", "")).strip()
                if venue_symbol:
                    return venue_symbol
        return None
    ...
    def validate_entry_constraints(
        ...
        *,
        ...
        if base_reason is not None:
            return base_reason
        if self.execution_disabled_reason:
            return self.execution_disabled_reason
        if not self.api_configured:
            return f"{self.broker_id}_not_configured"
        if not _proposal_is_trading212_sourced(proposal):
            return "trading212_market_data_required"
        symbol = str(proposal.get("symbol", "")).strip().upper()
        if self.equity_symbols and symbol not in self.equity_symbols:
            return "trading212_symbol_not_allowed"
        if self.primary_currency == "GBP" and float(usd_to_gbp or 0.0) <= 0:
            return "fx_reference_unavailable"
        if self.resolve_ticker(symbol, proposal=proposal) is None:
    ...
    def submit_order(
        ...
        *,
        ...
        """
        if self.execution_disabled_reason:
            raise BrokerAdapterError(self.execution_disabled_reason)
    ...
    def build_entry_order_request(
        ...
        *,
        ...
        symbol = str(proposal.get("symbol", "")).strip().upper()
        ticker = self.resolve_ticker(symbol, proposal=proposal)
        ...
        reference_price_native = _entry_price_in_primary_currency(
            proposal=proposal,
            fallback_price_usd=entry_price,
            primary_currency=self.primary_currency,
            usd_to_gbp=usd_to_gbp,
        )
    ...
    def build_exit_order_request(
        ...
        *,
        ...
        ticker = self.resolve_ticker(symbol, proposal=entry_order or latest_bar or {})
...
class Trading212LiveBrokerAdapter(Trading212PaperBrokerAdapter):
    """Trading 212 live account adapter, intentionally disabled for mutation.

    This gives Centaur a real broker identity for readiness/account wiring while
    keeping real-money Trading 212 orders fail-closed behind an explicit
    Trading-212-specific live switch and future approval.
    """

    broker_id = "trading212_live"
    label = "Trading 212 Live"
    state_prefix = "trading212_live"
    account_status = "ACTIVE"
    execution_disabled_reason = "trading212_live_disabled"

    @classmethod
    def from_config(cls, config: RuntimeConfig) -> "Trading212LiveBrokerAdapter":
        adapter = cls(
            api_key=config.trading212_live_api_key,
            api_secret=config.trading212_live_api_secret,
            base_url=config.trading212_live_base_url,
            timeout_seconds=config.trading212_live_request_timeout_seconds,
            primary_currency=config.trading212_live_primary_currency,
            ticker_overrides=config.trading212_live_ticker_overrides,
            equity_symbols=getattr(config, "trading212_live_equity_symbols", tuple()),
            api_configured=config.trading212_live_api_configured,
        )
        if bool(getattr(config, "trading212_live_execution_enabled", False)):
            adapter.execution_disabled_reason = "trading212_live_not_approved"
        return adapter
    ...
    def build_entry_order_request(self, *args: Any, **kwargs: Any) -> dict[str, Any]:
        raise BrokerAdapterError(self.execution_disabled_reason or "trading212_live_not_approved")

    def build_exit_order_request(self, *args: Any, **kwargs: Any) -> dict[str, Any]:
        raise BrokerAdapterError(self.execution_disabled_reason or "trading212_live_not_approved")
...
def _proposal_is_trading212_sourced(proposal: dict[str, Any]) -> bool:
    """Require Trading 212 entries to come from Trading 212 market evidence.

    Trading 212 has its own account/session boundary, so it must not execute a
    proposal produced from Alpaca bars just because the UK session is open.
    Until a Trading 212 market-data lane exists, this keeps the paper lane
    fail-closed instead of mixing broker clocks with another provider's bars.
    """
    provenance_values = {
        str(proposal.get(key, "")).strip().lower()
        for key in (
            "source",
            "data_provider",
        ...
        if str(proposal.get(key, "")).strip()
    }
    return any(value.startswith("trading212") for value in provenance_values)
...
def _entry_price_in_primary_currency(
    *,
    proposal: dict[str, Any],
    ...
    if str(primary_currency).upper() == "GBP":
        ...
        if entry_price_gbp is not None and entry_price_gbp > 0:
            return entry_price_gbp
    return _price_in_primary_currency(
