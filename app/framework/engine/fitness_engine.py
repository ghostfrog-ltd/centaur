...
2. `allocate_strategy_signals` joins those summaries back onto current signals
   ...
   or admitted by the paper/live-mirrored score-to-trade rule.
...
def allocate_strategy_signals(
    *,
    ...
    diagnostics so the dashboard can explain quiet ticks. The high-score
    override arguments are retained for config compatibility, but the execution
    policy is now deliberately simple: an approved strategy with a raw setup
    score at or above the configured score-to-trade threshold survives fitness
    suppression.
    """
    summary_index = {
        (
...
def _allocation_note(
    *,
    ...
    if allocation_status in {"high_score_override", "score_to_trade"}:
        return (
            f"Score-to-trade override: raw setup score >= {high_score_override_min_score:.1f}; "
            f"fitness composite {composite_score:.3f} is retained for ranking/reporting only "
            f"over {checkpoints_evaluated} checkpoints."
        )
    return (
