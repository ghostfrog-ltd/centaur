...
SAFETY_STATEMENT = (
    "Research-only specific replay cache precompute. No paper trades, live settings, "
...
class SpecificReplayCachePrecomputeReport:
    def __init__(
        self,
        *,
        base_strategy_id: str,
    ...
    def build_report(self) -> dict[str, Any]:
        ...
        result = self.variant_service.run_research(
            base_strategy_id=self.target.base_strategy_id,
            profile_id=self.target.profile_id,
            timeframe=self.target.timeframe,
            created_by="precompute_specific_replay_cache",
        ...
        report = {
            "title": "Specific Replay Cache Precompute",
            "prepared_at": prepared_at.isoformat(),
            "strategy": self.target.base_strategy_id,
            "profile": self.target.profile_id,
            "timeframe": self.target.timeframe,
        ...
        report["persistence"] = self._persist(prepared_at=prepared_at, report=report)
        return report
    ...
    @staticmethod
    def command_for_target(*, base_strategy_id: str, profile_id: str, timeframe: str) -> str:
        return (
            ".venv-mac/bin/python main.py --precompute-specific-replay-cache "
            f"--base-strategy {base_strategy_id} --profile-id {profile_id} --timeframe {timeframe}"
        ).strip()

    def _persist(self, *, prepared_at: datetime, report: dict[str, Any]) -> dict[str, Any]:
        raw = {
            "report_type": "replay_dataset_preparation",
            "scope": "strategy",
            "prep_status": (
                "replay_prepared_candidate_unlocked"
                if report["runtime_status"] != "runtime_blocked"
                else "replay_prepared_but_still_slow"
            ),
            "prep_action": "precompute_specific_replay_cache",
            "blocker_reason": report.get("runtime_blocker", ""),
            "base_strategy_id": self.target.base_strategy_id,
            "profile_id": self.target.profile_id,
            "timeframe": self.target.timeframe,
            "asset_class": "crypto" if self.target.base_strategy_id.startswith("crypto_") else "equity",
        ...
        evaluation_id = (
            ...
            f"{self.target.base_strategy_id}:{self.target.profile_id}:{self.target.timeframe}:{timestamp_fragment}:{digest}"
        )
        self.usage_ledger.record_strategy_variant_evaluation(
            evaluation_id=evaluation_id,
            variant_id="precompute-specific-replay-cache",
            base_strategy_id=self.target.base_strategy_id,
            profile_id=self.target.profile_id,
            timeframe=self.target.timeframe,
            replay_id=f"precompute-specific-replay-cache-{prepared_at.strftime('%Y%m%d-%H%M%S')}",
            ...
            drawdown=report.get("drawdown"),
            baseline_variant_id="",
            baseline_strategy_key=f"{self.target.base_strategy_id}/{self.target.profile_id}/{self.target.timeframe}",
