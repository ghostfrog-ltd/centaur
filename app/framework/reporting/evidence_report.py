...
class EvidenceReport:
    ...
    def _storage_separation_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "physical_split_status": (
                "schema_separated_shared_postgres"
                if backend == "postgres"
                else "dev_only_local_backend"
            ),
            "separation_enforcement": (
                "shared_core_plus_lane_schemas"
                if backend == "postgres"
                else "dev_only_local_backend"
            ),
            "rows_sampled": {
            ...
            "next_step": (
                "Run paper and live against the same PostgreSQL system if desired, "
                "but keep shared reviewed evidence in core and keep execution/account rows in lane schemas."
            ),
        }
    ...
    def _render_storage_separation_lines(self, storage: dict[str, Any]) -> list[str]:
        ...
        lines = [
            ...
            f"- separation_enforcement={storage.get('separation_enforcement', '-')}",
        ]
