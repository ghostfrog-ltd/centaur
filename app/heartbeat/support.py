"""Shared heartbeat step support.

Step orchestration lives under `app.heartbeat.steps`. This module keeps
cross-step helpers and framework adapter imports in one explicit place so
individual heartbeat step folders can own their actual runner bodies without
duplicating shared safety and execution helpers.
"""
...
from app.framework.adapters.execution import ExecutionAdapterError, get_execution_adapter
from app.framework.adapters.market_data import MarketDataAdapterError, get_market_data_adapter
from app.framework.runtime.execution_router import ExecutionRouter
from app.framework.runtime.mode_context import mode_context_from_config
...
from app.framework.engine.fitness_engine import allocate_strategy_signals, enrich_strategy_fitness_rows
from app.framework.engine.prediction_engine import GeminiApiError, get_gemini_client
...
def _paper_min_projected_gain_pct(config: Any, asset_class: str) -> float:
    """Return the paper gain floor for the asset class without widening risk."""
...
def _live_runtime_allows_broker_reads(context: TickContext) -> bool:
    """Central read gate for live broker state."""
...
def _live_runtime_allows_order_mutation(context: TickContext) -> bool:
    """Central mutation gate for live order side effects."""
...
def _broker_equity_market_snapshots(
    *,
    context: TickContext,
    account_trade_ready: bool,
    account_ready_reason: str,
    alpaca_market_open: bool,
    alpaca_clock: dict[str, Any],
) -> dict[str, dict[str, Any]]:
    """Expose broker-specific equity sessions without weakening global gates.

    The legacy equity scanner still uses Alpaca market data, so the top-level
    gate remains Alpaca/US-backed. Broker lane snapshots let CFO approvals make
    their own market-hours decision once a broker-specific proposal/data source
    exists, and keep the audit trail honest while unsupported lanes fail closed.
    """
    alpaca_ready = account_trade_ready and alpaca_market_open
    snapshots: dict[str, dict[str, Any]] = {
        "alpaca_paper": {
            "broker_id": "alpaca_paper",
            "market_open": alpaca_market_open,
            "equity_scan_ready": alpaca_ready,
            "reason": "market_open" if alpaca_ready else (
                account_ready_reason if not account_trade_ready else "market_closed"
            ),
            "timezone": getattr(context.config, "market_timezone", "America/New_York"),
            "source": "alpaca_clock",
            "next_open": alpaca_clock.get("next_open"),
            "next_close": alpaca_clock.get("next_close"),
        }
    }
    if _paper_trading212_enabled(context):
        trading212_account_ready, trading212_account_reason = (
            _trading212_paper_account_trade_ready(context)
        )
        configured_window = _configured_weekday_market_window(
        ...
        trading212_ready = trading212_account_ready and bool(configured_window["market_open"])
        snapshots["trading212_paper"] = {
            **configured_window,
            "broker_id": "trading212_paper",
            "equity_scan_ready": trading212_ready,
            "reason": "market_open" if trading212_ready else (
                trading212_account_reason
                if not trading212_account_ready
                else configured_window["reason"]
            ),
            "source": "configured_weekday_window",
        }
    return snapshots

def _trading212_paper_account_trade_ready(context: TickContext) -> tuple[bool, str]:
    ...
    if not isinstance(summary, dict) or not summary:
        return False, "trading212_paper_account_unavailable"
    status = str(summary.get("status", "")).upper()
    account_ready = status in {"ACTIVE", "DEMO"}
    trading_blocked = bool(
        summary.get("trading_blocked") or summary.get("account_blocked")
    )
    user_suspended = bool(summary.get("trade_suspended_by_user"))
    if not account_ready:
        return False, "trading212_paper_account_not_active"
    if trading_blocked:
        return False, "trading212_paper_account_blocked"
    if user_suspended:
        return False, "trading212_paper_user_trade_suspension"
    return True, "account_trade_ready"

def _configured_weekday_market_window(
    *,
    as_of: datetime,
    market_timezone: str,
    open_time: str,
    ...
    market_tz = ZoneInfo(str(market_timezone or "Europe/London"))
    local_now = as_of.astimezone(market_tz)
    open_at_time = _parse_clock_time(open_time, default=time(hour=8, minute=0))
    close_at_time = _parse_clock_time(close_time, default=time(hour=16, minute=30))
    today_open = local_now.replace(
        hour=open_at_time.hour,
        minute=open_at_time.minute,
        second=0,
        microsecond=0,
    )
    today_close = local_now.replace(
        hour=close_at_time.hour,
        minute=close_at_time.minute,
        second=0,
        microsecond=0,
    )
    ...
    market_open = is_weekday and today_open <= local_now < today_close
    if market_open:
        next_open = today_open
        next_close = today_close
        reason = "market_open"
    else:
        next_open = _next_weekday_open(
            local_now=local_now,
    ...
    return {
        ...
        "timezone": str(market_timezone or "Europe/London"),
        "session_date": local_now.date().isoformat(),
        "open_time": f"{open_at_time.hour:02d}:{open_at_time.minute:02d}",
        "close_time": f"{close_at_time.hour:02d}:{close_at_time.minute:02d}",
        "next_open": next_open.isoformat(),
        "next_close": next_close.isoformat(),
        "reason": reason,
    }

def _parse_clock_time(value: str, *, default: time) -> time:
...
def _next_weekday_open(*, local_now: datetime, today_open: datetime) -> datetime:
    if local_now.weekday() < 5 and local_now < today_open:
        return today_open
...
def _trading212_position_price_bars(
    *,
    context: TickContext,
    ...
    positions = list(context.state.get("trading212_paper_positions", {}).get("raw", []))
    bars: dict[str, dict[str, Any]] = {}
    for position in positions:
        if not isinstance(position, dict):
            continue
        ticker = _trading212_position_ticker(position)
        symbol = (
            _trading212_config_symbol_from_ticker(context.config, ticker)
            or _trading212_symbol_from_ticker(ticker)
        )
        ...
        if quantity in (None, 0.0) or current_price is None or current_price <= 0:
            continue
        quote_currency = _trading212_position_currency(position)
        bars[symbol] = {
            "t": str(position.get("updatedAt") or position.get("createdAt") or context.started_at.isoformat()),
            "o": current_price,
            "h": current_price,
            "l": current_price,
            "c": current_price,
            ...
            "canonical_instrument_id": str(
                position.get("canonical_instrument_id") or f"{symbol}-TRADING212-EQUITY"
            ),
            "venue": "trading212",
            "venue_symbol": ticker,
            "quote_currency": quote_currency,
            "source": "trading212_market_data",
            "data_provider": "trading212_market_data",
            "market_data_provider": "trading212_market_data",
        }
    return bars

def _trading212_position_ticker(position: dict[str, Any]) -> str:
    instrument = position.get("instrument")
    if isinstance(instrument, dict):
        ticker = str(instrument.get("ticker") or "").strip()
        if ticker:
            return ticker
    return str(position.get("ticker") or position.get("venue_symbol") or "").strip()

def _trading212_symbol_from_ticker(ticker: str) -> str:
...
def _trading212_position_currency(position: dict[str, Any]) -> str:
    instrument = position.get("instrument")
    if isinstance(instrument, dict):
        currency = str(
        ...
        if currency:
            return currency
    return str(position.get("currencyCode") or position.get("currency") or "GBP").strip().upper() or "GBP"
...
def _build_paper_trade_approval(
    *,
    ...
    if (
        ...
        and not _broker_equity_market_open(
        ...
        return None, {
            ...
            "broker_id": broker_id,
            "reason": _broker_equity_market_reason(
                market_gate=market_gate,
                broker_id=broker_id,
            ),
        }
...
def _broker_equity_market_state(
    *,
    market_gate: dict[str, Any],
    broker_id: str | None,
) -> dict[str, Any]:
    ...
    if isinstance(broker_markets, dict):
        broker_state = broker_markets.get(normalized)
        if isinstance(broker_state, dict):
            return broker_state
    return {
        "market_open": bool(market_gate.get("market_open")),
        "reason": market_gate.get("equity_reason") or market_gate.get("reason") or "market_closed",
        "next_close": market_gate.get("next_close"),
    }

def _broker_equity_market_open(
    *,
    market_gate: dict[str, Any],
    broker_id: str | None,
) -> bool:
    return bool(
        _broker_equity_market_state(
            market_gate=market_gate,
            broker_id=broker_id,
        ).get("market_open")
    )

def _broker_equity_market_reason(
    *,
    market_gate: dict[str, Any],
    broker_id: str | None,
) -> str:
    ...
    return reason or "market_closed"

def _broker_equity_next_close(
    *,
    market_gate: dict[str, Any],
    broker_id: str | None,
) -> Any:
    return _broker_equity_market_state(
        market_gate=market_gate,
        broker_id=broker_id,
    ).get("next_close")
...
def _position_symbol_for_broker(
    ...
    *,
    ...
    if symbol:
        return symbol
    venue_symbol = str(position.get("ticker") or position.get("venue_symbol") or "").strip()
    instrument = position.get("instrument")
    if isinstance(instrument, dict) and not venue_symbol:
        venue_symbol = str(instrument.get("ticker") or "").strip()
    if str(broker_id or "").strip().lower() == "trading212_paper":
        configured_symbol = _trading212_config_symbol_from_ticker(config, venue_symbol)
        if configured_symbol:
            return configured_symbol
        return _trading212_symbol_from_ticker(venue_symbol)
    return _symbol_from_broker_payload({**position, "ticker": venue_symbol})

def _trading212_config_symbol_from_ticker(config: Any | None, ticker: str) -> str:
    normalized_ticker = str(ticker or "").strip().upper()
    if not normalized_ticker or config is None:
        return ""
    overrides = getattr(config, "trading212_paper_ticker_overrides", {}) or {}
    for symbol, venue_ticker in overrides.items():
        if str(venue_ticker or "").strip().upper() == normalized_ticker:
            return str(symbol or "").strip().upper()
    return ""

def _trading212_price_seed_symbols(config: Any) -> set[str]:
    configured = getattr(config, "trading212_paper_price_seed_symbols", None)
    if configured is None:
        configured = getattr(config, "trading212_paper_equity_symbols", ())
    return {str(symbol).strip().upper() for symbol in configured or () if str(symbol).strip()}

def _has_managed_entry_order(
    *,
    symbol: str,
    broker_id: str,
    ...
    for order in orders:
        ...
        if order_symbol != symbol_upper and _normalized_symbol_key(order_symbol) != symbol_key:
            continue
        if _order_filled_qty(order) <= 0:
            continue
        if _order_has_managed_exit_plan(order):
            return True
    return False

def _is_trading212_price_seed_position(
    *,
    context: TickContext,
    position: dict[str, Any],
    broker_id: str,
    ...
    if str(broker_id or "").strip().lower() != "trading212_paper":
        return False
    symbol_upper = str(symbol or "").strip().upper()
    if not symbol_upper or symbol_upper not in _trading212_price_seed_symbols(context.config):
        return False
    ...
    if quantity is not None and quantity <= 0:
        return False
    # Seed positions are a Trading 212 API price-feed workaround. Once Centaur
    # has a filled managed buy for the same broker/symbol, the aggregated broker
    # position is managed normally and no longer exempt from exits or slot use.
    return not _has_managed_entry_order(
        symbol=symbol_upper,
        broker_id=broker_id,
        orders=recent_orders,
    )

def _paper_lane_position_state(
    context: TickContext,
    broker_id: str,
    *,
    recent_orders: list[dict[str, Any]] | None = None,
) -> dict[str, Any]:
    state_key = _positions_state_key_for_broker(broker_id)
    positions_state = context.state.get(state_key, {})
    summary = positions_state.get("summary", {}) if isinstance(positions_state, dict) else {}
    raw_positions = positions_state.get("raw", []) if isinstance(positions_state, dict) else []
    normalized_broker = str(broker_id or "").strip().lower()
    orders = recent_orders if recent_orders is not None else []

    if normalized_broker != "trading212_paper" or not isinstance(raw_positions, list):
        symbols = {
            str(symbol).strip().upper()
        ...
        return {
            ...
            "symbols": symbols,
            "price_seed_positions": 0,
            "price_seed_symbols": set(),
        }
    ...
    for position in raw_positions:
        if not isinstance(position, dict):
            continue
        symbol = _position_symbol_for_broker(
            position,
            normalized_broker,
            config=context.config,
        )
        ...
        if _is_trading212_price_seed_position(
            ...
            price_seed_positions += 1
            seed_symbols.add(symbol_upper)
            continue
        open_positions += 1
        managed_symbols.add(symbol_upper)

    return {
        "open_positions": open_positions,
        "symbols": managed_symbols,
        "price_seed_positions": price_seed_positions,
        "price_seed_symbols": seed_symbols,
    }
...
def _centaur_symbol_from_venue_symbol(value: str) -> str:
    normalized = str(value or "").strip()
    upper = normalized.upper()
    if "_US_EQ" in upper:
        return upper.split("_US_EQ", 1)[0]
    if "_" in normalized:
...
def _latest_raw_bars_by_key(context: TickContext) -> dict[tuple[str, str], dict[str, Any]]:
    ...
    source_map = {
        ...
        "trading212_data_latest_bars": "trading212_market_data",
    }
