"""Heartbeat step implementation owned by `32_execution_live`."""
...
def run_implementation(context: TickContext) -> PipelineResult:
    """Submit approved live entries and persist the live audit trail.

    Live execution only sends orders that survived the live CFO lane and the
    final real-money router guard, then stores Alpaca Live responses separately
    so live decisions and broker outcomes remain reviewable.
    """
            ...
            submitted_orders.append(
                {
                    **routed.order,
                    ...
                    "planned_profit_capture_pct": context.config.live_execution_profit_capture_pct,
                }
            )
        elif routed.error:
