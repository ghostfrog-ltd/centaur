"""Heartbeat step implementation owned by `34_notifications_slack`."""
...
def run_implementation(context: TickContext) -> PipelineResult:
    ...
    _sync_attention_alerts(context)
    if not alerts:
        repeat_result = send_due_attention_alerts(context=context)
        result = {
            "channel": "slack",
            "mode": "idle",
            "alerts_built": 0,
            "attention_alerts_sent": int(repeat_result.get("alerts_sent", 0) or 0),
            "attention_errors": int(repeat_result.get("errors", 0) or 0),
        }
        context.state["slack_notifications"] = result
        return result
    ...
    repeat_result = send_due_attention_alerts(context=context, sender=sender)
    result["attention_alerts_sent"] = int(repeat_result.get("alerts_sent", 0) or 0)
    result["attention_errors"] = int(repeat_result.get("errors", 0) or 0)
    context.state["slack_notifications"] = result
    return result
