...
- Architecture direction: adapter-first Centaur core with vendor-specific market-data, execution, broker/account, and symbol-mapping adapters; `centaur-codex-architecture-instructions.md` is the read-first guide for this refactor direction; current Alpaca latest-bar and historical backfill fetches now pass through the initial `centaur.market_data_adapters` boundary
- Broker and market data today: Alpaca Paper and Alpaca market data
- Execution architecture today: broker-adapter layer with Alpaca Paper active, Alpaca Live scaffolded/approved only inside the recorded same-as-paper follower envelope, and IG scaffolded
...
- Operations database: PostgreSQL; paper/live operation must not silently fall back to SQLite; optional `POSTGRES_SCHEMA` is honored for schema-level paper/live separation when explicitly configured
...
- Config loading: `CENTAUR_CONFIG` points at `configs/paper.yaml` or `configs/live.yaml` for lane defaults; `.env` remains the final operator override layer.
- FX conversion for GBP reporting: ECB daily reference feed
- Runtime model: pipeline-first control flow, designed to map cleanly to LangGraph, with a shared `ModeContext` for explicit `shadow`, `paper`, `live_dry`, and `live` mode boundaries; `live_dry` is live-environment but non-mutating
...
- the DDEV dashboard still reads `var/dashboard_snapshot.json` through its `/api/snapshot.php` route, but automatic refresh after each control tick is currently disabled in headless trading mode so dashboard work does not slow the trade loop; a separate `launchd` job refreshes the snapshot every `300` seconds with skip-if-busy locking, and manual snapshot generation remains available for operator checks
