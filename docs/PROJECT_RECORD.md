...
- Architecture direction: adapter-first Centaur core with vendor-specific market-data, execution, broker/account, and symbol-mapping adapters; `centaur-codex-architecture-instructions.md` is the read-first guide for this refactor direction; Alpaca latest-bar and historical backfill fetches now run through `app.adapters.market_data`
...
- Adapter model: Alpaca market-data fetches now pass through `app.adapters.market_data`, and order-request building plus order mutation now pass through `app.adapters.execution`; execution-adapter lookup is explicitly limited to `alpaca_paper` and `alpaca_live`, with non-Alpaca concrete adapters still pending. `.venv-mac/bin/python main.py --adapter-inventory` is the read-only inventory for active, bridged, scaffold-only, and not-implemented providers
