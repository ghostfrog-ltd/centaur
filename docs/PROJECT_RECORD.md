...
- Execution architecture today: broker-adapter layer with active paper lanes for Alpaca Paper and approved Trading 212 equities, an independent Alpaca Live lane over shared evidence/signals, and IG scaffolded
- Active live architecture: shared evidence/signals feed independent paper and live lanes. Paper applies `PAPER_*` `.env` dials; live applies `LIVE_*` `.env` dials exactly. Live does not blindly copy paper.
- Order routing today: approved entries, managed exits, and stale-order cancellations pass through `ExecutionRouter`; live entry/exit/cancel actions receive immediate `LiveRiskGuard` re-checks before broker mutation, including live activation, kill switch, account/sync readiness, broker identity, entry capacity, latest-bar availability, instrument/venue permission checks where derivable, and notional. Live entry selection uses audited live proposal/risk approval rather than same-paper entry dependency. `live_dry` records intended actions in tick state without calling the live broker adapter
...
- Config loading: `.env` is the single runtime config file, grouped into core, paper, and live sections so paper/live levers are visible without YAML indirection. `PAPER_*` governs paper, `LIVE_*` governs live, and reports show the exact active dials behind every lane decision.
...
- show an Alpaca Live lane for side-by-side paper/live operations
- show read-only live execution intelligence that compares same-proposal paper/live entries where both exist, plus unmatched live orders and blockers
...
- already allowed strategies may use the score-to-trade rule when raw `signal_score` is at or above the active configured dial (`PAPER_MIN_SIGNAL_SCORE_TO_TRADE` for paper, `LIVE_MIN_SIGNAL_SCORE_TO_TRADE` for live/live-dry; currently `96` for both lanes in `.env`); those candidates survive fitness suppression so the relevant lane CFO/risk gate can evaluate them. Paper and live use their own execution enable/kill-switch state, allowlists, margins, and score dials. This does not change notional, stops, projected-gain floors, broker routing, max orders, or final live-guard requirements
...
- Alpaca Live is approved as an independent live lane by explicit 2026-06-04 operator clarification, evolved from the 2026-05-29 same-as-paper first-live envelope
- The active live policy is independent live proposals controlled exactly by `LIVE_*` `.env` dials over shared evidence/signals. Live may trade/skip/block according to those live dials, not because paper submitted or did not submit the same order.
- Alpaca Live currently uses equities plus crypto, the same three allowed strategies, `$10` notional, `10` base slots, one order per tick, `$2.00` persisted daily protector, `1.5%` equity projected-gain, `2.0%` crypto projected-gain, `5` bps equity buffer, and `25` bps crypto buffer unless `.env` and explicit approval say otherwise
- The recorded first-live policy was same-as-paper by operator request, but extra account funds above `$100` are launch buffer only and do not create additional first-live slots
...
On 2026-05-20, the Alpaca Live lane moved from pure scaffold to a dormant side-by-side follower path. At that time, API keys alone were deliberately insufficient and live could only consider trades that the paper CFO gate approved on the same tick. This was superseded on 2026-06-04 by the independent `LIVE_*` dial lane, while the separate live account state, live daily-loss, live slot, live allowlist, kill-switch, activation acknowledgement, broker-validation gates, and managed-exit plan safety remain active before any entry order submission.
