...
- Execution architecture today: broker-adapter layer with active paper lanes for Alpaca Paper and approved Trading 212 equities, Alpaca Live currently approved only inside the recorded same-as-paper follower envelope, and IG scaffolded
- Target execution architecture: shared evidence/signals feed independent paper and live lanes. Paper applies `PAPER_*` `.env` dials; live applies `LIVE_*` `.env` dials exactly. Live should not blindly copy paper after the migration is implemented, tested, reported, and explicitly activated.
- Order routing today: approved entries, managed exits, and stale-order cancellations pass through `ExecutionRouter`; live entry/exit/cancel actions receive immediate `LiveRiskGuard` re-checks before broker mutation, including live activation, kill switch, account/sync readiness, broker identity, current same-paper-order validation, entry capacity, latest-bar availability, and instrument/venue permission checks where derivable. Future independent live-lane work should replace same-paper entry dependency with audited live proposal/risk approval while preserving `LiveRiskGuard`. `live_dry` records intended actions in tick state without calling the live broker adapter
...
- Config loading: `.env` is the single runtime config file, grouped into core, paper, and live sections so paper/live levers are visible without YAML indirection. The current same-as-paper follower runtime mirrors live execution levers to paper by default and requires named allowed differences while armed. The target independent-lane runtime should make this simpler: `PAPER_*` governs paper, `LIVE_*` governs live, and reports show the exact active dials behind every lane decision.
...
- Alpaca Live is approved for same-as-paper follower activation by explicit 2026-05-29 operator override and must not submit real-money entries outside those current go-live gates until the independent live lane is implemented, tested, reported, and explicitly activated
- The target live policy recorded on 2026-06-04 is independent live proposals controlled exactly by `LIVE_*` `.env` dials over shared evidence/signals. Live should be allowed to trade/skip/block according to those live dials, not because paper submitted or did not submit the same order.
- Alpaca Live currently mirrors paper settings for equities plus crypto, the same three allowed strategies, `$10` notional, `10` base slots, one order per tick, `$2.00` persisted daily protector, `1.5%` equity projected-gain, `2.0%` crypto projected-gain, `5` bps equity buffer, and `25` bps crypto buffer; target live-lane migration must keep these values unless `.env` and explicit approval say otherwise
